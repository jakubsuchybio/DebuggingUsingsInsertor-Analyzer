...
{
	/// <summary>
	/// Superclass of all Unit tests made for diagnostics with codefixes. Contains methods used to
	...
	public abstract partial class CodeFixVerifier : DiagnosticVerifier
	{
		...
		/// A bool controlling whether or not the test will fail if the CodeFix introduces other
		/// warnings after being applied
		/// </param>
		...
		/// <summary>
		/// General verifier for codefixes. Creates a Document from the source string, then gets
		/// diagnostics on it and applies the relevant codefixes. Then gets the string after the
		/// codefix is applied and compares it with the expected result.
		...
		void VerifyFix(string language , DiagnosticAnalyzer analyzer , CodeFixProvider codeFixProvider , string oldSource , string newSource , int? codeFixIndex , bool allowNewCompilerDiagnostics)
		{
			...
			for(int i = 0; i < attempts; ++i)
			{
				...
				{
					// Format and get the compiler diagnostics again so that the locations make
					// sense in the output
